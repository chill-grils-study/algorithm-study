...
// 5번 선수는 4위인 2번 선수에게 패배했기 때문에 5위입니다. [4,3,1] 2 5
//이진트리?
//부모 설정
//내가 이긴사람+진사람 = N-1개 --> 순번확실
// dfs(i, winGraph)   // i가 이길 수 있는 사람 수
// dfs(i, loseGraph)  // i에게 이길 수 있는 사람 수
function solution(n, results) {
    ...
    for(let i=1;i<n+1;i++){
        for(let j=1;j<n+1;j++){
            if(arr[i][j]){
                //i<j
                lose[i].push(j); //i입장에서 j에게 짐
                win[j].push(i); //j입장에서 i 이김
                //dfs, 정점 파고들어서 체크
            }
        }
    }
    ...
    for(let i=1;i<n+1;i++){
        ...
        dfs(i,lose,visited);
        const loseCnt = visited.filter(Boolean).length;
        visited = Array(n + 1).fill(false);
        dfs(i,win,visited);
        const winCnt = visited.filter(Boolean).length;
